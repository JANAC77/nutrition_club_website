      ...
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '2rem' }}>
          {servicesData.map((service, idx) => (
            <div key={idx} style={{ background: 'white', padding: '2rem 1.5rem', borderRadius: '15px', textAlign: 'center', boxShadow: 'var(--shadow-md)', transition: 'var(--transition)' }} className="service-card-new">
              <div style={{ width: '70px', height: '70px', borderRadius: '50%', background: service.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem auto' }}>
                {service.icon}
              </div>
              <h3 style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '0.8rem', color: 'var(--text-dark)' }}>{service.title}</h3>
              <p style={{ fontSize: '0.9rem', color: 'var(--text-gray)', lineHeight: '1.5' }}>{service.desc}</p>
            </div>
          ))}
        </div>
      </div>
